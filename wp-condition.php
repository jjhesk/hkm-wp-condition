...
Description: Enhanced by Hesk, Display WP-Condition in Chart for Database Performance, Memory Performance, Site Performance, and Social Performance. Requires PHP 5.2.0+
Version: 1.0.0
Author: Hesk
...
class WP_Page_Condition_Stats
{
    ...
    {
//		$this->display();
        add_submenu_page('tools.php', 'WP Condition', 'Memory Condition', 'manage_options', 'wp-conditions', array(&$this, 'display'));
    }
    ...
    function enqueue()
    {
        ...
        if (isset($current_screen))
            if ($current_screen->id == "tools_page_wp-conditions")
                wp_enqueue_script('wpfixit_con-script', plugins_url('Chart.min.js', __FILE__));
    }

    private function wpo_fs_info($filesize)
    {
        $bytes = array(
            'B',
            'K',
        ...
        <div id="wpfixit_container" style="width:100%">
            <div id="wpfixit_conditions">
                <table>
                    ...
                    <tr>
                        <td>
                            <h2>Database Perfomance:</h2>

                            <p><?php printf(__('%s queries in %s seconds.', 'wpfixit_con'), $query_count, $timer_stop); ?>
                                ...
                                $dbsize = $this->wpo_fs_info($dbsize);
                        ...
                        <td>
                            ...
                            <p><?php printf(__('Average load time of %s (%s runs).', 'wpfixit_con'), $average_load_time, sizeof($load_times)); ?></p>
                            ...
                            <div id="wpfixit_right" style="float:left">
                                ...
                                <form action="http://www.wpfixit.net/?page_id=168" method="post" target="_blank">
                                    ...
                                    <input name="username" type="hidden"
                                           value="<?php echo urlencode(bloginfo('name')) . '-' . $current_user->user_login ?>"/>
                                    <input name="performance" type="hidden"
                                           value="<?php echo 'MemoryPeak:' . $memory_peak_usage . 'MemoryUsage:' . $memory_usage . 'AvgLoadtime:' . $average_load_time . 'TotalQueries:' . $query_count . 'TimeforQueries:' . $timer_stop ?>"/>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="actions">
                                <a class="reset"
                                   href="<?php echo add_query_arg('reset_wpfixit_con_stats', 1); ?>">Reset</a>
                            </div>
                        </td>
                    </tr>
     ...
     * @return string
     */
    private function convert_bytes_to_hr($bytes)
    {
